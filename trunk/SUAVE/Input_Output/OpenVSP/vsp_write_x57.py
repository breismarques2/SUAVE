...
def vsp_write_x57(vehicle,tag):
    ...
    if vehicle.propulsors.has_key('propulsor'):
    
    
        # Unpack network
        network  = vehicle.propulsors.propulsor
        n_engines_forward = network.number_of_engines_forward
        n_engines_lift = network.number_of_engines_lift
        
        # Unpack Forward Propeller
        prop_forward = network.propeller_forward
        t_radius_forward=prop_forward.tip_radius
        
        # Design Forward Propellers
        disk_id = vsp.AddGeom("Disk")
        vsp.SetGeomName(disk_id, 'cruise_propeller')
        area_tags[network.tag] = ['disk',network.tag]
        
        mid_span=(vehicle.wings.main_wing.spans.projected)/2.0
        
    
        vsp.SetParmVal(disk_id,"X_Rel_Location","XForm",vehicle.wings.main_wing.origin[0]+mid_span*math.tan(vehicle.wings.main_wing.sweeps.leading_edge*math.pi/180.0)-0.1*(vehicle.wings.main_wing.chords.root))
        vsp.SetParmVal(disk_id,"Y_Rel_Location","XForm",(vehicle.wings.main_wing.spans.projected)/2.0)
        vsp.SetParmVal(disk_id,"Z_Rel_Location","XForm",vehicle.wings.main_wing.origin[2]+mid_span*math.tan(vehicle.wings.main_wing.dihedral*math.pi/180.0))
        ...
        while i<n_engines_lift/2:
        
           disk_id = vsp.AddGeom("Disk")
           vsp.SetGeomName(disk_id, 'high_lift_propeller_'+str(i+1))
           area_tags[network.tag] = ['disk',network.tag]
           ...
           vsp.SetParmVal(disk_id,"Y_Rel_Location","XForm",y_pos_lift)
           vsp.SetParmVal(disk_id,"Z_Rel_Location","XForm",z_pos_lift)
           vsp.SetParmVal(disk_id,'Diameter','Design',t_radius_lift*2.0)
           vsp.SetParmVal(vsp.GetParm(disk_id, "Sym_Planar_Flag", "Sym"), vsp.SYM_XZ)
